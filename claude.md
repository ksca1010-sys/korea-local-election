...
### Step 4: CLAUDE.md 작성 (30분)

프로젝트 루트에 이 파일을 만들면 Claude Code가 매 세션마다 자동으로 읽는다.
...
**제1조 (출처 의무)** 모든 수치 데이터(득표율, 의석수, 여론조사 등)는 반드시 공식 출처로 검증한 뒤 반영한다.
...
- 자동화 파이프라인에서 LLM이 생성한 후보/득표 데이터는 선관위 API로 사후 검증 (fetch → LLM 분석 → NEC API 교차검증 → 저장)
...
7. 선거 유형이 다른 API 데이터를 혼합하지 않음 (sgTypecode 구분)
...
## 4. Claude Code에서의 실행 명령어

이 작업을 Claude Code에서 하려면, 새 세션을 열고 아래 순서대로 지시:

```
1번째 지시:
"프로젝트 루트에 CLAUDE.md를 만들어줘. 내용은 [위 CLAUDE.md 내용 붙여넣기]"
...
| Claude Code가 전체 app.js를 건드림 | CLAUDE.md 규칙 덕에 해당 탭 파일만 수정 |
